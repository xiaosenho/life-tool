...
- `devices` / `push`：客户端安装实例、厂商推送标识与离线推送。
...
1. **前台在线**：客户端登录后建立 `GET /api/friends/events/stream` SSE 长连接。
2. **消息发送**：仍由 HTTP `POST /api/friends/messages/{friendUserId}` 提交。
3. **服务端广播**：新消息、好友申请、申请处理、已读回执通过 SSE 推给目标用户。
4. **离线补偿**：当目标用户当前没有活跃 SSE 连接时，后端尝试通过阿里云移动推送发送通知。
5. **设备绑定**：客户端登录后调用 `/api/devices` 上报 `installationId`、`vendorDeviceId`、`pushProvider`。

补充约定：

- 好友申请页同样复用这条 SSE 事件流，收到新申请或申请状态变更时前端即时刷新。
- 纪念日提醒复用同一套阿里云推送能力，但由独立的服务端提醒调度器触发。
- 纪念日提醒的业务最小粒度是“天”，默认策略为“每小时补扫一次，但同一天同一提醒只推一次”。

当前实现为单实例内存 broker，适合现阶段部署。后续若后端扩成多实例，推荐把好友事件广播迁移到 Redis Pub/Sub。
...
- 本地提醒可由客户端调度；服务端保存规则，并可通过阿里云推送执行离线提醒，用于多设备恢复。
- 周年和倒数天数由客户端展示，服务端提供即将到来的事件列表。
- 服务端同时返回 `displayDate` 和 `reminderOffsetDays`，支持“提前 N 天提醒实例”在日历中直接落位。
- 服务端提醒调度默认读取 `LIFETOOL_PUSH_REMINDERS_SCAN_DELAY_MS`，默认值 `3600000` 毫秒，即每小时补扫一次。
